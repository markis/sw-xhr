...
export type EventListener = (this: XMLHttpRequest, ev: XMLHttpRequestEventMap[EventListenerTypes]) => any;
...
export default class XMLHttpRequestPolyfilleEvents extends XMLHttpRequestPolyfillBase {
  ...
  public set onerror(listener: (this: XMLHttpRequest, ev: ErrorEvent) => any) {
  ...
  public removeEventListener(type: EventListenerTypes, listener: EventListener, options?: EventListenerOptions): void {
    switch (type) {
      case 'readystatechange':
        this.events.readystatechange = this.events.readystatechange.filter(e => e !== listener);
        break;
      case 'abort':
